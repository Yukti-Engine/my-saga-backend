...
/* ----------------- DB SIMULATION ----------------- */
const organisersTable: { email: string; phone: string; reason: string; password: string }[] = [];
const bossesTable: { email: string; phone: string; reason: string; password: string }[] = [];
...
export const organiserLogin = async (req: Request, res: Response) => {
  ...
  if (!organiser) return res.status(404).json({ error: "Organiser not found or not approved yet" });
...
export const bossLogin = async (req: Request, res: Response) => {
  ...
  if (!boss) return res.status(404).json({ error: "Boss not found or not approved yet" });
...
/* ----------------- APPROVE ORGANISER ----------------- */
export const approveOrganiser = async (req: Request, res: Response) => {
  const { email, securityKey } = req.body;
  if (securityKey !== "Babycorn") return res.status(403).json({ error: "Invalid security key" });

  const pending = pendingOrganisers.find((o) => o.email === email);
  if (!pending) return res.status(404).json({ error: "Pending organiser not found" });
  ...
  if (index !== -1) pendingOrganisers.splice(index, 1);
...
/* ----------------- APPROVE BOSS ----------------- */
export const approveBoss = async (req: Request, res: Response) => {
  const { email, securityKey } = req.body;
  if (securityKey !== "Babycorn") return res.status(403).json({ error: "Invalid security key" });

  const pending = pendingBosses.find((b) => b.email === email);
  if (!pending) return res.status(404).json({ error: "Pending boss not found" });

  const password = randomBytes(8).toString("hex");
  bossesTable.push({ ...pending, password });

  const index = pendingBosses.findIndex((b) => b.email === email);
  if (index !== -1) pendingBosses.splice(index, 1);
