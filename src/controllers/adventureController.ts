...
export const count = async (req: Request, res: Response) => {
  ...
  const check = await pool.query(
    `SELECT is_related_to_adventure($1, $2, $3) AS ok`,
    [id, role, adventureId]
  );
  if (!check.rows[0].ok)
    return res.json({ success: false });

  const result = await pool.query(`SELECT count_messages($1) AS count`, [adventureId]);
...
export const getMessages = async (req: Request, res: Response) => {
  ...
  const result = await pool.query(
    `SELECT * FROM get_messages_from_a_to_b($1, $2, $3)`,
    [adventureId, a, b]
  );
...
export default function roomSocket(io: any, socket: any) {
  socket.on("join_room", async ({ roomName, id, role, accessToken }: any) => {
    ...
    const roomOk = await pool.query(`SELECT room_available($1) AS ok`, [roomName]);
...
export async function getUploadFileUrl(req: any, res: any) {
  ...
  const countRes = await pool.query(`SELECT file_count($1) AS count`, [adventureId]);
