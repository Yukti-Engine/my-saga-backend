...
export async function insertResult(req: any, res: any) {
  ...
  if (!authResult.rows[0].is_authenticated)
    return res.status(500).json({ error: "Authentication Error" });
  const resultQuery = await pool.query(`SELECT insert_result($1::int, $2::int[], $3::text[], $4::int, $5::text) AS poll_number`, [adventureId, badgeIds, userIds, starScores, remarks]);
