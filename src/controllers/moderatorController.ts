...
import { escapeHtml, validateName, validatePhone, validateEmail, validateBoundedText, validateHttpUrl, validatePositiveInt, validateFutureTimestamp, validateFloatRange } from "../validators.js";
...
export const addVenuePartner = async (req: Request, res: Response) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT create_venue_partner($1::varchar, $2::varchar, $3::varchar) AS id`,
      [nameV.value, phoneV.value, emailV.value]
    );
    return res.json({ success: true, id: rows[0].id });
  } catch (err: any) {
    if (err?.code === "23505")
      return res.status(409).json({ error: "A venue partner with this email already exists" });
    console.error("Error in addVenuePartner:", err);
    return res.status(500).json({ error: "Failed to add venue partner" });
  }
};

export const getVenuePartners = async (_req: Request, res: Response) => {
  const { rows } = await pool.query(`SELECT * FROM get_venue_partners()`);
  return res.json(rows);
};

export const removeVenuePartner = async (req: Request, res: Response) => {
  const idV = validatePositiveInt(req.body.id, "id");
  if (!idV.ok) return res.status(400).json({ error: idV.error });

  const { rows } = await pool.query(`SELECT delete_venue_partner($1::int) AS found`, [idV.value]);
  if (!rows[0].found) return res.status(404).json({ error: "Venue partner not found" });
  return res.json({ success: true });
};
...
export const addVenue = async (req: Request, res: Response) => {
  const nameV = validateBoundedText(req.body.venueName, "venueName", 2, 255);
  if (!nameV.ok) return res.status(400).json({ error: nameV.error });

  const linkV = req.body.venueLink
    ? validateHttpUrl(req.body.venueLink, "venueLink", 2000)
    : { ok: true as const, value: null };
  if (!linkV.ok) return res.status(400).json({ error: linkV.error });

  const latV = req.body.venueLat != null
    ? validateFloatRange(req.body.venueLat, "venueLat", -90, 90)
    : { ok: true as const, value: null };
  if (!latV.ok) return res.status(400).json({ error: latV.error });

  const longV = req.body.venueLong != null
    ? validateFloatRange(req.body.venueLong, "venueLong", -180, 180)
    : { ok: true as const, value: null };
  if (!longV.ok) return res.status(400).json({ error: longV.error });

  const partnerIdV = req.body.venuePartnerId != null
    ? validatePositiveInt(req.body.venuePartnerId, "venuePartnerId")
    : { ok: true as const, value: null };
  ...
  try {
    ...
    if (err?.code === "23503")
      return res.status(400).json({ error: "Venue partner not found" });
    console.error("Error in addVenue:", err);
...
export const removeVenue = async (req: Request, res: Response) => {
  const idV = validatePositiveInt(req.body.venueId, "venueId");
  if (!idV.ok) return res.status(400).json({ error: idV.error });

  const { rows } = await pool.query(`SELECT delete_venue($1::int) AS found`, [idV.value]);
  if (!rows[0].found) return res.status(404).json({ error: "Venue not found" });
  return res.json({ success: true });
};
