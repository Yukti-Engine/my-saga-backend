...
export default function roomSocket(io, socket) {
    socket.on("join_room", async ({ roomName, id, role, accessToken }) => {
        let person;
        if (role == "organizer")
            person = await getOrganizer(id, pool);
        else if (role == "boss")
    ...
    socket.on("send_message", async ({ room, senderId, senderType, accessToken, message }) => {
        let person;
    ...
    socket.on("leave_room", async ({ roomName, id, role, accessToken }) => {
        let person;
        if (role == "organizer")
            person = await getOrganizer(id, pool);
        else if (role == "boss")
