-- Remove min_team_members from match_requests; adventure threshold is now statically 6.

DROP FUNCTION IF EXISTS public.create_match_request(
  integer, integer, double precision, integer, integer, integer,
  double precision, double precision, double precision, boolean, boolean, text, integer
);
...
ALTER TABLE match_requests DROP CONSTRAINT IF EXISTS chk_min_team_members_range;
ALTER TABLE match_requests DROP COLUMN IF EXISTS min_team_members;

CREATE OR REPLACE FUNCTION public.create_match_request(
  p_org_id integer,
  p_category_id integer,
  p_match_radius double precision,
  p_age_range_min integer,
  p_age_range_max integer,
  p_latitude double precision,
  p_longitude double precision,
  p_pay_per_head double precision,
  p_all_girls boolean,
